...
@defproc[(callback-connection-status [my-tox _Tox-pointer] [anonproc procedure?]
                                [userdata cpointer?]) void?]{
  ...
  anonproc is in the form
    @racket[(anonproc tox friendnumber status userdata)]
...
@defproc[(tox-do-interval [tox _Tox-pointer]) integer?]{
  ...
  returns time (in ms) before the next @tt{tox-do} needs to be run on success.
}
